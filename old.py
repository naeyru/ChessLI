...
class ChessBoard:
    ...
    def is_attacked(self, pos, team):  # Is a square held by team being attacked?
        attackers = []
        for piece in self.get_visible_pieces(pos):
            if team != piece[0].team:
                attackers.append(piece)

        return attackers

    def is_check(self, team):  # Is team in check?
        pos = ""
        for r in ranks:
            for f in files:
                if self.board[f"{f}{r}"].name == "king" and self.board[f"{f}{r}"].team == team:
                    pos = f"{f}{r}"
                    break

        if self.is_attacked(pos, team):
            return True
    ...
    def get_legal_moves(self, team):
        ...
        for r in ranks:
            for f in files:
                if self.board[f"{f}{r}"].team == team:
                    moves.append((f"{f}{r}", self.board[f"{f}{r}"].visible_squares(f"{f}{r}")))
        return moves

    def is_checkmate(self, team):
        if not self.is_check(team):
            return False
        ...
        for move in self.board[pos].visible_squares(pos):
            new_board = self
            new_board.board[pos] = ChessManEmpty()
            new_board.board[move] = self.board[pos]
            if not new_board.is_check(team):
                return False

        all_legal_moves = self.get_legal_moves(team)

        for move in all_legal_moves:
            for sub in move[1]:
                new_board = self
                new_board.board[move[0]] = ChessManEmpty()
                new_board.board[sub] = self.board[move[0]]
                if not new_board.is_check(team):
                    return False
...
def main():
    ...
    game_board.board["d1"] = ChessManQueen("black")
    print(game_board.is_checkmate("white"))
